...
# Check if the source directory exists
    ...
    if (Test-Path $sourceDirectory -PathType Container) {
        ...
        $files = Get-ChildItem $sourceDirectory | Where-Object { $_.PSIsContainer -eq $false -and $_.LastWriteTime -lt $archiveThreshold -and [System.IO.Path]::GetExtension($_.FullName) -eq '.001' }
        ...
        Start-Process -FilePath $winrarPath -ArgumentList "a -r- -df -x*.rar -ep1 $archiveName $sourceDirectoryF " -Wait
